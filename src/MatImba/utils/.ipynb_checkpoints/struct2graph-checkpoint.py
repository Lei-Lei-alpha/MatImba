...
class SimpleCrystalConverter:
    ...
    def convert(self, d):
        ...
        center_indices, neighbor_indices, _, distances = find_points_in_spheres(
            cart_coords, cart_coords, r = self.cutoff,
            pbc = pbc, lattice = lattice_matrix, tol = 1e-8
        )
        ...
        y = getattr(d, 'y') if hasattr(d, 'y') else 0
        omega = getattr(d, "omega", None)
        phi = getattr(d, "phi", None)
        rou = getattr(d, "rou", None)
                
        bond_batch = MyTensor(np.zeros(edge_index.shape[1])).long()
        
        return Data(
            x = x, edge_index = edge_index, edge_attr = edge_attr,
            state = torch.Tensor(state), y = y, omega = omega,
            phi = phi, rou = rou, bond_batch = bond_batch
        )
