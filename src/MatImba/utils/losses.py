...
def calc_alpha(labels: torch.Tensor, preds: torch.Tensor, densities: torch.Tensor) -> torch.Tensor:
    ...
    # Statistics. dcov2 can be negative — do NOT pre-clamp here (see SmoothDILALoss
    # above); clamp dcor2 after normalisation instead so the sign/magnitude of an
    # anti-correlated (desired) state is preserved through the division.
    dcov2 = (A_centered * B_centered).mean(dim=(1, 2))
    dvar_x2 = torch.clamp((A_centered * A_centered).mean(dim=(1, 2)), min=1e-12)
    dvar_y2 = torch.clamp((B_centered * B_centered).mean(dim=(1, 2)), min=1e-12)

    dcor2 = dcov2 / (torch.sqrt(dvar_x2 * dvar_y2) + 1e-8)
    dcor = torch.sqrt(torch.clamp(dcor2, min=0.0) + 1e-12)
