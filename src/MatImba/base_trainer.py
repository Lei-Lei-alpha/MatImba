"""Model-agnostic robust training for imbalanced regression.

This module contains :class:`BaseRobustTrainer`, the generic training loop
behind MatImba's robust model selection.  It is deliberately independent of
any model architecture or data format: to reuse the robust training method
(DILA loss, EMA-smoothed robust score, tail-error resistant checkpointing)
with your own PyTorch model, subclass it and implement two hooks:
...
``weights`` (sample weights), ``density`` (local label density) and
``relevance`` (tail relevance function phi) are optional; metrics that need a
missing field fall back to uniform values.  See
``examples/custom_trainer_example.py`` for a complete runnable example.
"""
...
class BSAM(torch.optim.Optimizer):
    ...
    This optimizer wraps a base optimizer (e.g., AdamW) to perform a two-step
    optimization that minimizes both the loss value and the loss landscape sharpness.

    Algorithm:
    1. Compute Gradient at current weights w.
    2. Ascent: Move w to w_adv (adversarial point) in the direction of greatest loss increase.
    3. Compute Gradient at w_adv.
    4. Descent: Update original w using the gradient from w_adv.
    """
    def __init__(self, params, base_optimizer, rho=0.05, adaptive=False, **kwargs):
        ...
        # CRITICAL: Sync param_groups!
        # Standard optimizers create their own copies of param_groups.
        # We must point the wrapper's param_groups to the inner optimizer's
        # param_groups so that external Scheduler updates are reflected immediately.
    ...
    def second_step(self, zero_grad=False):
        """
        Perform the Descent Step: Restore w, update using gradient from w + e
        """
        for group in self.param_groups:
            for p in group["params"]:
                if p.grad is None: continue
        ...
        # Update using the gradients currently stored in p.grad (which were computed at w+e)
    ...
    @torch.no_grad()
    def _grad_norm(self):
        # Fail-safe for empty params or device mismatch during initialization
...
class BatchFields(NamedTuple):
    """Per-batch supervision fields returned by :meth:`BaseRobustTrainer.unpack_batch`.

    Attributes:
        y: Target values, shape ``(N,)`` or ``(N, C)``.
        weights: Optional sample weights (omega) for weighted losses.
        density: Optional local label density (rou) used by DILA and awareness.
        relevance: Optional tail relevance phi(y) in [0, 1] used by SERA.
    """
    y: torch.Tensor
...
class BaseRobustTrainer:
    """
    A model-agnostic robust trainer for imbalanced regression.

    This trainer handles the full lifecycle: training loops, validation,
    metric tracking (MAE, MSE, SERA, R2, awareness alpha), robust-score
    checkpoint selection, CSV logging, and visualization.

    Subclasses must implement :meth:`forward_batch` and :meth:`unpack_batch`
    (and may override :meth:`move_batch` for non-tensor batch types).

    Args:
        model (nn.Module): The PyTorch model to train.
        train_loader: Iterable DataLoader for the training set.
        val_loader: Iterable DataLoader for the validation set.
        test_loader (optional): Iterable DataLoader for the test set.
        ...
        profile_runtime (bool, optional): Whether to track time and memory per batch.
        save_checkpoints (bool, optional): Whether to save checkpoints and prediction CSVs.
    """
    def __init__(
        self,
        model: nn.Module,
    ...
    # ------------------------------------------------------------------
    # Model/data hooks — the only methods a subclass must provide.
    # ------------------------------------------------------------------
    def move_batch(self, batch: Any) -> Any:
        """Moves a batch to the training device. Override for exotic batch types."""
        return batch.to(self.device, non_blocking=True)

    def forward_batch(self, batch: Any) -> torch.Tensor:
        """Runs the model on a batch and returns raw predictions."""
        raise NotImplementedError("Subclasses must implement forward_batch(batch).")

    def unpack_batch(self, batch: Any) -> BatchFields:
        """Extracts targets and optional weights/density/relevance from a batch."""
        raise NotImplementedError("Subclasses must implement unpack_batch(batch).")
    ...
    # Training / evaluation
    # ------------------------------------------------------------------
    def train(self, epoch: int, dataloader: Optional[Any] = None) -> Tuple[float, float]:
        ...
        self.model.train()
        end = time.time() # Reverted to simple time() to avoid CUDA event overhead causing issues

        # Minimal change: profile data accumulator
        profile_data = []

        # DIL Warmup logic (Keep as is)
        ...
        for idx, batch in enumerate(dataloader):
            ...
            fields = self.unpack_batch(batch)
            n_samples = fields.y.size(0)

            # Helper closure to calculate loss (needed twice for BSAM)
            def compute_loss():
                ...
                if self.weighted_loss:
                    if fields.weights is None:
                        raise ValueError(
                            "weighted_loss=True but unpack_batch() returned no weights."
                        )
                    current_weights = fields.weights
                ...
                current_rou = None
                if self.dil_inform:
                    if fields.density is None:
                        raise ValueError(
                            "dil_inform=True but unpack_batch() returned no density."
                        )
                    current_rou = fields.density
                    ...
                    loss, penalty = self.loss_func(outputs, y, current_rou, weights=current_weights)
                    return loss, penalty
                elif self.weighted_loss:
                    return self.loss_func(outputs, y, current_weights), 0.0
                else:
                    return self.loss_func(outputs, y), 0.0
            ...
            # Profiling step: Capture memory and time
            if self.profile_runtime:
                torch.cuda.synchronize(self.device)
                batch_end_time = time.perf_counter()
                mem_mb = torch.cuda.max_memory_reserved(self.device) / (1024**2)
                elapsed_ms = (batch_end_time - batch_start_time) * 1000
                profile_data.append({
                    "epoch": epoch, "batch": idx,
                    "time_ms": elapsed_ms, "peak_mem_mb": mem_mb,
                    "method": "BSAM" if is_bsam else "Standard"
                })

            # Logging
            self.meter_train_loss.update(loss.item(), n_samples)
            if self.meter_train_penalty:
                # Handle tensor vs float penalty
                pval = penalty_val.item() if isinstance(penalty_val, torch.Tensor) else penalty_val
                self.meter_train_penalty.update(pval, n_samples)
            ...
            # Simple progress log
            if idx % 10 == 0:
                logger.info(f"Epoch: [{epoch}][{idx}/{len(dataloader)}] Loss {self.meter_train_loss.avg:.4f}")

        # Save profiling to CSV if enabled
        if self.profile_runtime and profile_data:
            profile_path = os.path.join(self.outdir, f"{self.name}_runtime_profile.csv")
            pd.DataFrame(profile_data).to_csv(profile_path, mode='a', header=not os.path.exists(profile_path), index=False)

        if hasattr(self.model, "update_fds"): self.model.update_fds(dataloader, epoch)
    ...
    def validate(self, prefix: str = "Val", dataloader: Any = None) -> Tuple[float, ...]:
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(dataloader):
                ...
                # Batch-level metrics
                loss_l1 = self.criterion_l1(outputs, y)
                self.meter_val_l1.update(loss_l1.item(), y.size(0))
                loss_mse = self.criterion_mse(outputs, y)
                self.meter_val_mse.update(loss_mse.item(), y.size(0))
                ...
                all_losses_raw.append((outputs - y).abs())
                ...
                dens = fields.density if fields.density is not None else torch.ones_like(y)
                phi = fields.relevance if fields.relevance is not None else torch.ones_like(y)
        ...
        # SERA integrated from t=0.5 to 1.0. The monitoring bound is deliberately
        # looser than the extreme-value analysis bound (t0=0.8): at t0=0.8 the
        # integral is dominated by a handful of extreme, often noisy OOD samples,
        # and coupling checkpoint selection to it destabilises training and
        # promotes underfitting of the bulk. t=0.5 keeps the monitor stable while
        # still emphasising the tail; it matches the SI performance tables.
        ...
        alpha_func = naiive_calc_alpha if self.alpha_metric == "pcc" else calc_alpha
    ...
    def fit(self):
        ...
        try:
            for epoch in self.epoch_range:
                ...
                # 2% Safety Gate:
                # We strictly REJECT any "Robust" checkpoint if its MAE is > 2% worse than best seen.
    ...
    def predict(self, dataloader: Any) -> Tuple[np.ndarray, ...]:
        """
