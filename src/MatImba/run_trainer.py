...
from MatImba.trainer import CgcnnTrainer, LossExplosionError, BSAM
...
class RobustObjective:
    ...
    def __call__(self, trial):
        ...
        # Save rho to user_attrs so we can retrieve it later
        if bsam_rho: trial.set_user_attr("bsam_rho", bsam_rho)
...
def run(config_path, config_file):
    ...
    if use_bsam:
        logger.info("BSAM Active: Enforcing Weighted Loss and Log-Inverse Weights")
    ...
    for fold in expt_config["data"]["folds"]:
        ...
        for run_id in range(multi_runs):
            ...
            if best_params_for_runs:
                # 1. Try to find rho in 'hpo_params' (Matches your JSON structure)
                if 'hpo_params' in best_params_for_runs:
                    rho = best_params_for_runs['hpo_params'].get('rho', None)
                
                # 2. Fallback: Try top-level 'bsam_rho' (Legacy/Alternative structure)
                if rho is None:
                    rho = best_params_for_runs.get('bsam_rho', None)
                ...
                # Defaults
                # Fallback if no HPO
                if use_bsam and rho is None:
                    rho = expt_config.get('bsam', {}).get('rho', 0.05)
                    logger.info(f"BSAM active but no rho in params. Using config default: {rho}")
            ...
            if rho is not None:
                base_cls = get_obj(expt_config['optimiser']['name'])
                optimiser = BSAM(model.parameters(), base_optimizer=base_cls, rho=rho, **optim_params)
